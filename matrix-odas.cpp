...
extern bool uartopen();
extern bool uartout(int8_t data);
extern void uartclose();
static int8_t lastangle45 = 0;
static int8_t angle45 = 0;
static bool angleUpdated = true;
void *anglepoll(void *ptr)
{
  while (1) {
    if(angleUpdated)
      // send minus angle45 since the robot rotation direction is the inverse of matrix angle
      if(uartout(-angle45) == false) printf("error sending angle %d via UART\n", angle45);
      else printf("sent angle %d via UART, energy = %lf\n", angle45, E);
    angleUpdated = false;
    sleep(2);
  }
  return NULL;
...
void increase_pots() {
  ...
  static double aveE = 0.0;
  static double outAngle = 0;
  if (E > maxE) {maxE = E; outAngle = angle_xy;}
  static unsigned aveCnt = 0;
  if(E > 0.2) {aveE += E; aveCnt ++;}
  if (cnt % 1000 == 0)
  {
    if (maxE > 0.5 && aveE > 0.38 && aveCnt > 200) {
      printf("angle = %d, maxEnergy = %f, aveEnergy = %f, aveCnt=%d\n", (int)outAngle, maxE, aveE/(aveCnt+1), aveCnt);
      angleUpdated = true;
      if (outAngle >= 0 && outAngle <= 45) angle45 = (int)outAngle;
      else if (outAngle > 45 && outAngle <= 180) angle45 = 45;
      else if (outAngle > 180 && outAngle < 315) angle45 = -45;
      else angle45 = (int)outAngle - 360;
      lastangle45 = angle45;
    }
    maxE = 0.0;
    aveE = 0.0;
...
int main(int argc, char *argv[]) {
  ...
  pthread_t anglepoll_thread;
  if(pthread_create(&anglepoll_thread, NULL, anglepoll, NULL)) {
  ...
  if(pthread_join(anglepoll_thread, NULL)) {
